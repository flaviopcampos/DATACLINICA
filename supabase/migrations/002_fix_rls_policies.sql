-- Fix RLS policies - Remove existing and recreate correctly
-- This migration fixes issues with the previous RLS implementation

-- Drop existing policies that may have been partially created
...
-- Medications policies (reference table - read for all authenticated users)
CREATE POLICY "medications_read_all" ON medications
    FOR SELECT USING (auth.role() = 'authenticated');

CREATE POLICY "admins_manage_medications" ON medications
...
-- Product Categories policies (reference table)
CREATE POLICY "product_categories_read_all" ON product_categories
    FOR SELECT USING (auth.role() = 'authenticated');

CREATE POLICY "admins_manage_product_categories" ON product_categories
    FOR INSERT WITH CHECK (is_current_user_admin());

CREATE POLICY "admins_update_product_categories" ON product_categories
    FOR UPDATE USING (is_current_user_admin());

CREATE POLICY "admins_delete_product_categories" ON product_categories
...
-- Medical Record Diagnosis policies
...
-- CID Diagnosis policies (reference table - read for all authenticated users)
